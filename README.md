...
- **M1.3 — в работе.** Реализованы сервисные плагины браузера: вкладки (`browser.tabs`), долговременная история (`browser.history`), загрузки (`browser.downloads`) с непрозрачными артефактами, куки и данные сайтов (`browser.cookies`, `browser.site-data`) со строгой изоляцией контекстов и redacted-секретами, а также проверочные срезы S3A и S3B.
...
### Сервисы браузера (M1.3)

- `worldline-browser-services-contract` — движко-независимые экспериментальные
  контракты уровня v0.1: `browser.tabs`, `browser.history`, `browser.downloads`,
  `browser.cookies` и `browser.site-data`.
- `worldline-browser-tabs` — сервисный плагин управления вкладками пользователя
  (`TabId`, группировка, закрепление, упорядочивание, выбор) с семантикой detach-only.
- `worldline-browser-history` — сервисный плагин долговременной истории навигации
  с идемпотентной дедупликацией и асинхронным обогащением заголовками.
- `worldline-browser-downloads` — сервисный плагин загрузок с долговременным
  `DownloadRecordId`, фиксацией намерения до вызова движка, непрозрачными `ArtifactRef`
  (без раскрытия путей хоста) и сверкой после сбоя.
- `worldline-browser-cookies` — сервисный плагин инспекции метаданных, раскрытия
  секретных значений с redacted-представлением в логах, строгой изоляцией контекстов
  и точечной очисткой хранилищ сайтов.
...
- [Архитектура сервисных плагинов браузера](docs/adr/ADR-BROWSER-SERVICE-PLUGINS-V1.md) —
  сервисы вкладок, истории, загрузок, куки/данных сайтов и срезы S3A/S3B.
